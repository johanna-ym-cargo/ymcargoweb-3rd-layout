...
Six self-contained HTML/CSS page templates built from the real content and
...
| `services.html` | **Catalog / detail page**, tag cloud, sticky anchor sub-nav, alternating detail sections with feature sub-cards, CTA banner, quote-request panel | Services, Industries, Equipment |
...
| `contact.html` | **Contact page**, quick-reach tiles, two-column contact panel + form, embedded map | Contact, Support, Branch locator |

All six share the same `<header>` (top contact/social bar + sticky nav with
mobile drawer) and `<footer>` (offices grid, affiliated companies, verse,
legal line) markup, so adding a seventh page is a matter of copying the
head/header/footer block from any existing page and dropping a new
`<main>` built from the component classes in `assets/css/style.css`. The
photo-backed contact/quote panel (`.career-contact-card` /
`.quote-card-split`) is itself a shared component reused as-is across
`careers.html`, `contact.html`, and `services.html`.

## Structure

```
├─ index.html / about.html / services.html / news.html / careers.html / contact.html
├─ assets/
│  ├─ css/
...
and breakpoints at `480px`, `640px`, `760-860px`, `900-991px`, and
`1080px+`. The nav collapses into a full-height slide-in drawer under
`992px`; the services sub-nav becomes a horizontally-scrollable strip on
small screens; the footer's offices grid drops to a single column and its
verse divider switches from a side border to a top border below `760px`.
...
ymcargo.ph pages (Home, About, Services, News, Careers, Contact) as of
August 2026. The Careers job listings are clearly labeled as **sample**
data. Wire up a real ATS/job feed before shipping.
